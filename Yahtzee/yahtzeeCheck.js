...
// the below code used for create random values
    (pointLock1 == 0) ?   rand1 = 6 : null;
    (pointLock2 == 0) ?   rand2 = 6 : null;
    (pointLock3 == 0) ?   rand3 = 6 : null;
    (pointLock4 == 0) ?   rand4 = 6 : null;
    (pointLock5 == 0) ?   rand5 = 6 : null;
...
// The below function is for changing the coustume of the dice
    ...
    if(oneTimeAction[7] == 1){
        document.getElementById("pl1Yahtzee").innerText = 100+scoreForyahtzeepl1;
        scoreForyahtzeepl1 += 100;
        oneTimeAction[7] = 2;
        // init();
    }
...
rollCount++;
    if(rollCount >= 3){  
        //initializing all variable 
            ...
            roll1.style.opacity=".5";
            roll1.style.cursor = "not-allowed";
            // roll2.style.cursor = "pointer";
...
function init(){
    ...
    document.getElementById("turn").style.top = "24%"

    // roll1.style.visibility = "hidden";
    // roll2.style.visibility = "visible";

    roll1.style.opacity = ".5";
    roll2.style.opacity = "1";
...
function scoreLocForpl1(idName){
    if(scoreLock == 1){
        ...
        if(totalCheck == 14 && sumCountForPl1 == 6 && sumCountForPl2 == 6){
            let total1 = onesLock+twosLock+threeLock+foursLock+fivesLock+sixLock+chanceLock+scoreForFullpl1+scoreForLargepl1+scoreForSmallpl1+scoreForyahtzeepl1+fourOfKindScore+threeOfKindScore+bonusForPl1;
            document.getElementById("pl1Total").innerText = total1;
            let total2 = onesLock2+twosLock2+threeLock2+foursLock2+fivesLock2+sixLock2+chanceLock2+scoreForFullpl2+scoreForLargepl2+scoreForSmallpl2+scoreForyahtzeepl2+fourOfKindScore2+threeOfKindScore2+bonusForPl2;
...
//  The below function is for rolling the dice for player 2
 function rollDicePl2(){
    ...
    pointLock1ForPl2 == 0 ? rand1For2 = 6 : null;
    pointLock2ForPl2 == 0 ? rand2For2 = 6 : null;
    pointLock3ForPl2 == 0 ? rand3For2 = 6 : null;
    pointLock4ForPl2 == 0 ? rand4For2 = 6 : null;
    pointLock5ForPl2 == 0 ? rand5For2 = 6 : null;
// the below function is for calculating all the values
    ...
    if(oneTime2[12] == 1){
        document.getElementById("pl2Yahtzee").innerText = 100+scoreForyahtzeepl2;
        scoreForyahtzeepl2 += 100;
        oneTime2[12] = 2;
        // init2();
    }
...
rollCount++;
    if(rollCount >= 3){
        ...
        roll2.style.opacity=".5";
        roll2.style.cursor = "not-allowed";
...
function init2(){
    ...
    roll2.style.opacity = ".5";
    roll1.style.opacity="1";
    roll2.style.cursor = "not-allowed";
    roll1.style.cursor = "pointer";

    document.getElementById("turn").style.top = "79.9%"
    ...
    if(scoreLock2 == 1){
        ...
        if(sumCountForPl2 == 6){
            if(onesLock2+twosLock2+threeLock2+foursLock2+fivesLock2+sixLock2 >= 63){
                bonusForPl2 = 35;
                document.getElementById("pl2Bonus").innerText = bonusForPl2; 
            }
            else{
                bonusForPl2 = 0;
                document.getElementById("pl2Bonus").innerText = bonusForPl2; 
